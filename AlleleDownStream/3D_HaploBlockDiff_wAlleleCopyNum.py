# -*- coding: utf-8 -*-
"""
Created on Fri Apr 18 17:06:36 2025
...
def parse_options():
    ...
    parser.add_argument("--input_copynum",default = None,
                        help = "path to copy number file derived from WGS data")
...
def build_contact_vector(allele1_record,allele2_record,start,end,res,maxrangebin,smbin,copy1,copy2):
    start_pos = start
...
def write_output(current_chrom,blockstart,blockend,start,end,tmp_pos,tmp_ref,tmp_allele1,tmp_allele2,
                 totalcount1,totalcount2,copy1,copy2,sparse_mat1,sparse_mat2,contact_mat,diff_record,out):
    ...
    if len(diff_record) > 0:
        for idx in range(filtered.shape[0]):
            outline2 = out_str[0:13]+ [str(int(filtered[idx,0])),
                                       str(int(filtered[idx,1])),
                                       str(int(filtered[idx,2])),
                                       str(int(filtered[idx,3])),
                                       "%.3e"% filtered[idx,4]]
...
def process_block(block_records,contacts,res,contact_mat,maxrangebin,reshaploblock,smbin,copy_chrom,copy_pos,out):
    current_chrom = ""
            ...
            if len(pos) >= 2:
                ...
                for idx in range(bin_size):
                    ...
                    if len(tmp_pos) >= 1:
                        ...
                        #block bin copy number
                        f1 = (copy_chrom == current_chrom)
                        f2 = (copy_pos[:,0] == start) & (copy_pos[:,1] == end)
                        copy_bin = copy_pos[f1 & f2,2:4]
                        if totalcount1 > 0 and totalcount2 > 0 and copy_bin.shape[0] == 1:
                            copy1 = copy_bin[0,0]
                            copy2 = copy_bin[0,1]
                            if copy1 > 0 and copy2 > 0:
                                sparse_mat1,sparse_mat2,diff_record = build_contact_vector(allele1_record,allele2_record,start,end,res,maxrangebin,smbin,copy1,copy2)
...
if __name__ == "__main__":
    ...
    #allele specific copy num 
    copynum = pd.read_csv(options.input_copynum,sep = "\t",header = 0)
    copy_chrom = np.array(copynum["chrom"].tolist(),dtype= "U")
    copy_pos = np.array(copynum[["bin_start","bin_end","allele1_copyNumber","allele2_copyNumber"]],dtype = "int64")
    #haplotype block
    blocks = open(options.input_haplotype_block,"r")
    block_records = blocks.readlines()
    #output files
    contact_mat = open(options.output_path + "_haplotypeblock_mat_WGScopy.txt","w")
    ...
    out = open(options.output_path + "_haplotypeblock_diff_WGScopy.txt","w")
    headers = ["chrom","phaseblock_start","phaseblock_end","bin_start","bin_end","mutPos_thisbin",
               "ref","allele1","allele2","total_contact1","total_contact2","copy1","copy2",
    ...
    process_block(block_records,contacts,options.resolution,contact_mat,options.maxrangebin,
                  options.resolutionblock,options.mergedbin,copy_chrom,copy_pos,out)
    contact_mat.close()
    blocks.close()
    out.close()
